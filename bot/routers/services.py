...
async def handle_apply_def_loc(callback: CallbackQuery, session: AsyncSession, settings: Settings) -> None:
  ...
  # Clean, professional HTML formatted string
    success_text = (
        ...
        f"1️⃣ دی‌ان‌اس‌های جدید بالا را جایگزین DNS قبلی دستگاه خود کنید.\n"
        f"2️⃣ موبایل و کنسول/سیستم خود را به یک اینترنت مشترک وصل کنید.\n"
        f"3️⃣ فیلترشکن و پروکسی تلگرام خود را خاموش کنید.\n"
        f"4️⃣ روی دکمه <b>«ثبت آی‌پی اتوماتیک»</b> در زیر کلیک کنید.\n\n"
        f"💡 <i>نکته: شما می‌توانید لوکیشن DNS را بعد از خرید به تعداد نامحدود تغییر دهید.</i>\n\n"
        f"⚠️ <i>اگر پس از تغییر DNS اتصال برقرار نشد، یک بار روی دکمه «ثبت آی‌پی اتوماتیک» زیر کلیک کنید و مجدداً وضعیت اتصال را بررسی کنید.</i>"
    )
