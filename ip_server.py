...
async def get_bot_username() -> str:
    ...
    if _bot_username is None:
        ...
        except Exception: _bot_username = "bot"
...
def _failed_html(reason: str, bot_username: str = "bot") -> HTMLResponse:
    return _render_capture_ip_html("تراکنش ناموفق", "❌ تراکنش ناموفق بود", reason, False, bot_username=bot_username)

def _success_html(message: str, bot_username: str = "bot") -> HTMLResponse:
    return _render_capture_ip_html("تراکنش موفق", "✅ پرداخت با موفقیت انجام شد", message, True, bot_username=bot_username)

# ============================================================================
# ROOT ROUTES (Fixing 404 Errors)
# ============================================================================
...
# ============================================================================
# IP CAPTURE & DASHBOARD ROUTES (FIXED UUID HYPHENS)
# ============================================================================
...
async def capture_or_dashboard_ip(request: Request, token: str):
    """Unified route for both /capture-ip/ and /ip/."""
    bot_user = await get_bot_username()
    token = token.strip()
    
    # UUID Fix: Create a hyphenated version of the token for the database query
    ...
    async with async_session_maker() as session:
        # Use OR condition to match both formats (with or without hyphens)
        stmt = (
            select(IPAuthToken)
        ...
        if not token_record or not token_record.service:
            return _render_capture_ip_html("خطا", "لینک نامعتبر", "این لینک وجود ندارد یا منقضی شده است.", False, bot_username=bot_user)
        ...
        if now > expires_at:
            return _render_capture_ip_html("خطا", "انقضای لینک", "مهلت استفاده از این لینک گذشته است.", False, bot_username=bot_user)

        service = token_record.service
        
        # If user is using the old /capture-ip format, run update immediately
        ...
        # If user is using the new /ip/ format, render the Shelter Dashboard HTML
        ...
        try:
            ...
            # Fallback if user_panel.html doesn't exist
            return _render_capture_ip_html("پنل کاربری", "✅ اتصال برقرار شد", "برای ثبت آی‌پی روی ثبت دستی کلیک کنید.", True, client_ip, bot_user)


@app.post("/api/ip/{token}/update")
async def api_update_ip(request: Request, token: str):
    """AJAX endpoint for dashboard IP update."""
    token = token.strip()
    formatted_token = token
    try:
        if len(token) == 32: formatted_token = str(uuid.UUID(token))
    ...
    async with async_session_maker() as session:
        ...
        if service.authorized_ip == client_ip:
            return JSONResponse(status_code=200, content={"success": True, "client_ip": client_ip, "message": f"آی‌پی {client_ip} هم‌اکنون فعال است."})

        success = await update_device_ip_safe(session, service, client_ip)
        if success:
            return JSONResponse(status_code=200, content={"success": True, "client_ip": client_ip, "message": f"آی‌پی {client_ip} با موفقیت ثبت شد."})
        else:
...
# ============================================================================
# PAYSTAR REDIRECT & CALLBACK (FIXED)
# ============================================================================
...
async def paystar_redirect(request: Request):
    ...
    if not token: 
        return _failed_html("توکن پرداخت یافت نشد.", bot_username=bot_user)
    ...
    try:
        ...
        async with async_session_maker() as session:
            ...
            if not payment or not payment.order or not payment.user:
                return _failed_html("توکن پرداخت منقضی شده است.", bot_username=bot_user)
            if payment.status == PaymentStatus.APPROVED.value or payment.order.status == OrderStatus.COMPLETED.value:
                return _success_html("این سفارش قبلاً با موفقیت پرداخت شده است.", bot_username=bot_user)

        html_content = f"""<!DOCTYPE html><html lang="fa" dir="rtl"><head><meta charset="UTF-8"><meta name="referrer" content="origin" /><title>در حال انتقال...</title></head><body style="background:#0f172a;color:#fff;text-align:center;font-family:Tahoma;padding-top:100px;">
        <h3>در حال انتقال به درگاه بانکی...</h3>
        <form id="paymentForm" action="https://core.paystar.click/api/pardakht/payment" method="POST"><input type="hidden" name="token" value="{clean_token}"><button type="submit" id="fallbackBtn" style="display:none;padding:10px 20px;background:#3b82f6;color:#fff;border:none;border-radius:5px;cursor:pointer;">انتقال دستی</button></form>
        <script>document.getElementById("paymentForm").submit(); setTimeout(function() {{ document.getElementById("fallbackBtn").style.display = "inline-block"; }}, 2500);</script></body></html>"""
        return HTMLResponse(content=html_content)
    except Exception as exc:
        return _failed_html(f"خطای داخلی: {str(exc)}", bot_username=bot_user)
...
async def paystar_callback(request: Request):
    ...
    try:
        ...
        async with async_session_maker() as session:
            ...
            context = await _build_paystar_context(order, service, settings) if service else {}
            if service: await _send_paystar_success_message(order, payment, context)
